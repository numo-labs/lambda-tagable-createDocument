...
exports.handler = function (event , context){
    ...
    if (!event._id) {return context.fail(new Error('no _id provided'))};
...
internal.processEvent = function(event, cb){
    ...
    AwsHelper.DynamoDB.putItem({
            TableName: tableName,
            Item: {
                _id: {S: doc._id.split('').reverse().join('')}, //reverse the id, better for distribution over the dynamodb cluster
                displayName: {
                    S: doc.displayName
                },
                activeTags: {
